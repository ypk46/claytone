...
export async function generateClaudeDir(config: ProjectConfig): Promise<void> {
  ...
  if (config.useMemoryFiles) {
    ...
    const content = interpolate(loadTemplate('templates/memory/design-decisions.md'), {
      date: today,
  ...
  if (config.includeHooks) {
    ...
    const written = await writeFile(hookPath, loadTemplate('templates/hooks/pre-tool-use.sh'));
    if (written) {
      logCreated(hookPath);
